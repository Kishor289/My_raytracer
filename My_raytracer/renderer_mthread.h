        ...
        if(file_.is_open()){
            ...
            for(int j=0; j<c.img_height; ++j){
                for(int i=0; i<c.img_width; ++i){
                    ...
                    for_each(execution::par, s_itr.begin(), s_itr.end(), [&]()
                    {
                        double r_ = c.focal_length * tan(c.d_focus_angle);

    
                        vec3 disk_point = c.look_from + rand_disk_point(r_, c.look_at - c.look_from,delta_x.normalized(), delta_y.normalized());
